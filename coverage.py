...
nPart = 100
numOrdNodes = len(ordNodes)
X = [[0 for y in range(len(ordNodes))] for x in range(nPart)] #encoding function - array which stores 0 or 1. 0 means node is in sleep state. 1 means it is awake
...
cov_s_without_s=[min(cov_p_without_s[i]) for i in range(len(ordNodes))] #same as above but without including s


#function to find the coverage
def find_cov():
    # for i in range(len(ordNodes)):
    #     ordNodes[i].ordInd=i        #assigning index to each ordinary node 
    ...
    for i in range(len(R_particle)):  #traverse through the list R_particle
        ...
        for j in R_particle[i]: #traverse through each list in R_particle
            temp=temp*(1-cov_s_p[i][j.ordInd]) #formula given in the paper to find overlap of the point
        cov_p.append(1-temp)
    ...
    for i in range(numOrdNodes): #traverse through the list of ordinary nodes
        for j in range(nPart):  #traverse through the list of particles
            ...
            for k in R_particle[j]: #traverse through the list of R_particles
                if k.ordInd!=i:       #if the node is not equal to itself
                    temp=temp*(1-cov_s_p[j][k.ordInd]) #find the overlap
